...
<body>
    ...
    <script>
        $(document).ready(function() {
            ...
            $.ajax({
                type: 'GET',
                url: '<%= request.getContextPath() %>/fetchUpcomingTrips',
                dataType: 'json',
                success: function(response) {
                    if (response.response_status.status === "success") {
                ...
                $('.booking-card').click(function() {
                    ...
                    if (passengerDetailsDiv.is(':visible')) {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.response_status.status === "success") {
                                    displayPassengerDetails(ticketNumber, response.data, passengerDetailsDiv.data('status-id'));
                                } else {
            ...
            function displayPassengerDetails(ticketNumber, passengers, statusId) {
                ...
                html += '</table>';
                if (statusId !== 2) {
                    html += '<button class="cancel-trip-button" data-ticket-number="' + ticketNumber + '">Cancel Trip</button>';
                }
                $('#passenger-' + ticketNumber).html(html).show();

                $('.cancel-trip-button').click(function() {
                    const ticketNum = $(this).data('ticket-number');
                    console.log(ticketNum);
                    if (confirm('Are you sure you want to cancel this trip?')) {
                        $.ajax({
                            type: 'POST',
                            url: '<%= request.getContextPath() %>/cancelTrip',
                            ...
                            success: function(response) {
                                if (response.response_status.status === "success") {
                                    ...
                                    alert('Trip cancellation failed: ' + response.response_status.message);
                                }
                            },
                            error: function(error) {
